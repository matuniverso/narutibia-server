...
function onSay(player, words, param)
	...
	if not reloadType then
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Reload inválido.")
		return false
	end

	Game.reload(reloadType)
	local capitalized = param:sub(1, 1):upper() .. param:sub(2)
	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, string.format("%s reloaded.", capitalized))
	return false
end
