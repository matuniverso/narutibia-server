...
if Modules == nil then
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
        ...
        outfit.lookType = 1263
        player:setOutfit(outfit)
        npcHandler:say("Aqui vamos nós!", cid)
        npcHandler:releaseFocus(cid)
        ...
        local function changeOutfitBack()
            outfit.lookType = currentOutfit
            player:setOutfit(outfit)
            player:teleportTo(destination)
        end

        addEvent(changeOutfitBack, duration * speed + 1)
		npcHandler:resetNpc(cid)
		return true
	end
	...
	function TravelModule.onConfirm(cid, message, keywords, parameters, node)
		...
		if player:isPremium() or not shop_premium[cid] then
			...
			elseif player:isPzLocked(cid) then
				...
				destination:sendMagicEffect(13)
			end
		else
